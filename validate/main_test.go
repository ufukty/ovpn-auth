...
func loadTestDatasetStoredSecrets() []file.StoredSecrets {
...
// internal use for tests
func loadTestDatasetUserInputs() []file.UserInput {
	content, err := os.ReadFile("test_data_valid.yml")
	if err != nil {
		log.Fatal(err)
	}
	user_inputs := []file.UserInput{}
...
// internal use for tests
func compare_datasets_on_final_method(stored_secrets []file.StoredSecrets, invalid_inputs []file.UserInput, valid_inputs []file.UserInput, test *testing.T) {
	...
	_ = Final(stored_secrets[0], invalid_inputs[0])
	...
	for i := 0; i < len(valid_inputs); i++ {
		...
		start := time.Now()
		_ = Final(stored_secrets[i], valid_inputs[i])
	...
	for i := 0; i < len(invalid_inputs); i++ {
		start := time.Now()
		_ = Final(stored_secrets[i], invalid_inputs[i])
